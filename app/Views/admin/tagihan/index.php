...
<div class="space-y-8" x-data="{ showModal: false, selected: {}, search: '<?= $search ?? '' ?>' }">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-surface-900">Daftar Tagihan</h1>
            <p class="text-surface-500 mt-1">Kelola semua tagihan pembayaran dari user.</p>
        </div>
        ...
        <div class="relative group">
            ...
            <input type="text"
                x-model="search"
                @keyup.enter="window.location.href = '<?= base_url('admin/tagihan') ?>?search=' + search"
    ...
    <div class="bg-white rounded-3xl border border-surface-100 shadow-xl shadow-surface-200/40 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-surface-50">
                    <tr>
                        <th class="px-6 py-4 text-left text-[10px] font-extrabold text-surface-400 uppercase tracking-widest">ID Tagihan</th>
                        ...
                        <th class="px-6 py-4 text-left text-[10px] font-extrabold text-surface-400 uppercase tracking-widest">Total Bayar</th>
                ...
                <tbody class="divide-y divide-surface-50">
                    <?php if (empty($tagihan)): ?>
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-surface-400 italic">Belum ada tagihan found.</td>
                        </tr>
                    <?php else: ?>
                            ...
                            <tr class="hover:bg-surface-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] text-surface-400"><?= $t['invoice_number'] ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-sm font-extrabold text-emerald-600">Rp<?= number_format($t['total_bayar'], 0, ',', '.') ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <?php
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <a href="<?= base_url('admin/tagihan/edit/' . $t['id']) ?>" class="p-2 text-primary-600 bg-primary-50 rounded-lg hover:bg-primary-600 hover:text-white transition-all">
                                        ...
                                        <a href="<?= base_url('admin/tagihan/hapus/' . $t['id']) ?>" onclick="return confirm('Hapus tagihan ini?')" class="p-2 text-rose-600 bg-rose-50 rounded-lg hover:bg-rose-600 hover:text-white transition-all">
        ...
        <div class="px-6 py-4 border-t border-surface-50">
            <?= $pager->links('tagihan', 'tailwind_pagination') ?>
        </div>
    </div>
    ...
    <div x-show="showModal"
        ...
        <div @click.outside="showModal = false"
            ...
            <div class="px-8 py-6 border-b border-surface-50 flex items-center justify-between bg-surface-50/50">
                <div>
                    <h3 class="text-xl font-bold text-surface-900">Detail Tagihan</h3>
                    <p class="text-sm text-surface-500 font-medium mt-0.5" x-text="'Inv: ' + selected.invoice_number"></p>
                </div>
            ...
            <div class="p-8 max-h-[70vh] overflow-y-auto scrollbar-hide">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex justify-between items-center text-sm">
                                    ...
                                    <span class="font-extrabold text-emerald-600" x-text="'Rp' + new Intl.NumberFormat('id-ID').format(selected.total_bayar)"></span>
                                </div>
                                <div class="flex justify-between items-center text-sm">
                        ...
                        <div>
                            <label class="text-[10px] font-extrabold text-surface-400 uppercase tracking-widest block mb-2">Informasi Waktu</label>
                            <div class="space-y-3 text-sm">
                                ...
                                <div class="flex justify-between">
                                    <span class="text-surface-500">Kadaluarsa</span>
                                    <span class="font-bold text-rose-500" x-text="selected.expiry_time"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-3 text-sm">
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <span class="font-semibold text-surface-700" x-text="selected.customer_whatsapp || '-'"></span>
                                </div>
                            </div>
                        </div>
            ...
            <!-- Modal Footer -->
            <div class="px-8 py-6 bg-surface-50 border-t border-surface-100 flex items-center justify-between">
                <div x-show="selected.payment_link">
                    <a :href="selected.payment_link" target="_blank" class="text-xs font-bold text-primary-600 hover:text-primary-700 flex items-center gap-1">
